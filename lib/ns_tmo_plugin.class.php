...
class NS_TMO_Plugin {
	...
	public function add_column_header( $columns ) {
		...
		$columns['menu_order'] = esc_html__( 'Order', 'term-menu-order' );
	...
	public function add_edit_menu_order( $term_id ) {
		...
		if ( isset( $_POST[ self::$form_field_name ] ) ) {
			...
			$wpdb->update( $wpdb->terms, array(
					'menu_order' => $_POST[ self::$form_field_name ],
				), array(
					'term_id' => $term_id,
				)
			);
	...
	*/
	...
	public function activate() {
		...
		$wpdb->query( $wpdb->prepare( 'ALTER TABLE `%s` ADD `menu_order` INT (11) NOT NULL DEFAULT 0;', $wpdb->terms ) );
